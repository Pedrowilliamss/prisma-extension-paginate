...
describe("Cursor", async () => {
	...
	it("should be possible to paginate the second page if the limit is equal to 1", async () => {
		const [, meta1] = await prisma.user.paginate({
			cursor: {
				limit: 1
			}
		})

		const [, meta2] = await prisma.user.paginate({
			cursor: {
				after: meta1.endCursor!,
				limit: 1
			}
		})

		expect(meta2).toMatchObject({
			hasNextPage: true,
			hasPreviousPage: true
		})
	})
